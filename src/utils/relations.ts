...
export function getGalleryVideos(galleryPath: string): string[] {
  return dataUtils.data.videoGalleries
    .filter((vg) => vg[1] === galleryPath)
    .map((vg) => vg[0]);
}

export function getTagVideos(tagTitle: string): string[] {
  return dataUtils.data.videoTags
    .filter((vt) => vt[1] === tagTitle)
    .map((vt) => vt[0]);
}

export function updateVideoGalleries(
  videoPath: string,
  diffObj: IDiffObj
): void {
...
export function updateGalleryVideos(
  galleryPath: string,
  diffObj: IDiffObj
): void {
